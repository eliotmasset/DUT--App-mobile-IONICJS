...
const helpers = require('./helpers-b13df8c9.js');
const index$1 = require('./index-b4269f8a.js');
const cubicBezier = require('./cubic-bezier-0b2ccc35.js');
const theme = require('./theme-30b7a575.js');
const frameworkDelegate = require('./framework-delegate-9daf084f.js');
  ...
  componentDidLoad() {
    {
      rIC(async () => {
        ...
        if (!ionicGlobal.config.getBoolean('_testing')) {
          Promise.resolve().then(function () { return require('./tap-click-840fb332.js'); }).then(module => module.startTapClick(ionicGlobal.config));
        }
        if (ionicGlobal.config.getBoolean('statusTap', isHybrid)) {
          Promise.resolve().then(function () { return require('./status-tap-2fa4da2b.js'); }).then(module => module.startStatusTap());
        }
        if (ionicGlobal.config.getBoolean('inputShims', needInputShims())) {
          Promise.resolve().then(function () { return require('./input-shims-c60ab6d6.js'); }).then(module => module.startInputShims(ionicGlobal.config));
        }
...
const RouterOutlet = class {
  ...
  async connectedCallback() {
    this.gesture = (await Promise.resolve().then(function () { return require('./swipe-back-5f141a67.js'); })).createSwipeBackGesture(this.el, () => !!this.swipeHandler && this.swipeHandler.canStart() && this.animationEnabled, () => this.swipeHandler && this.swipeHandler.onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
      if (this.ani) {
